...
Field Name | REQUIRED | Type | Defines | Mapping Notes
---|---|---|---|---
`stations` | Yes | Array | Array that contains one object per station as defined below. | /locations<br /><br />JSON response contains array, each representing a station. Only those wit the following attributes will be considered:<ul><li>deleted=false</li><li>extended/PublicCarsharing/hasPublicCarsharing=true</li></ul>
...
\-&nbsp;`address` | OPTIONAL | String | Address (street number and name) where station is located. This MUST be a valid address, not a free-form text description. Example: 1234 Main Street | `station['streetName']`
