...
`lat` | `station['lat']`
`lon` | `station['lon']`
`address` | `station['address']['street'] + ' ' + station['address']['houseNo']` (change requested to adhere to GBFS standard)
`cross_street` | -
`region_id` | -
`post_code` |  `station['address']['plz']` (change requested to adhere to GBFS standard)
`city` |  `station['address']['city']` (change requested to adhere to GBFS standard). Note: `city` will be introduced with GBFS v3.1, we publish it nevertheless already with GBFSv2.3
`rental_methods` | -
`is_virtual_station`| -
`station_area` | -
`parking_type` | -
